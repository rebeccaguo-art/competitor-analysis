...
def _build_filters(origination, months, competitor, preapproved=False, table_alias=""):
...
def get_available_competitors(cursor):
...
def get_overall_market_share(cursor, origination=None, months=None, preapproved=True, top_n=None):
    n = top_n or TOP_COMPETITORS_OVERALL
...
def get_preapproved_market_share(cursor, origination=None, months=None, preapproved=True, top_n=None):
...
def get_avg_rank(cursor, origination=None, months=None, preapproved=True, top_n=None):
    n = top_n or TOP_COMPETITORS_RANK
    where = _build_filters(origination, months, None, preapproved=True)
    sql = f"""
        WITH ranked AS (
        ...
        SELECT month, competitor, avg_rank FROM ranked WHERE rn <= {n}
        ORDER BY month, avg_rank ASC
    """
    return run_query(cursor, sql)


def get_avg_shelf_space(cursor, origination=None, months=None, preapproved=True, top_n=None):
    n = top_n or TOP_COMPETITORS_SHELF
    where = _build_filters(origination, months, None, preapproved=True)
    sql = f"""
        WITH pages AS (
        ...
        SELECT month, competitor, avg_shelf_space FROM agg WHERE rn <= {n}
...
def get_shelf_space(cursor, origination=None, months=None, competitor=None):
...
def get_cooccurrence_matrix(cursor, origination=None, months=None):
    ...
    sql = f"""
        ...
        WHERE {where_a} AND {where_b}
          AND a.podium_rank <= 5 AND b.podium_rank <= 5
...
def get_cooccurrence_detail(cursor, comp_a, comp_b, origination=None, months=None):
    where_a = _build_filters(origination, months, None, preapproved=True, table_alias="a")
    where_b = _build_filters(origination, months, None, preapproved=True, table_alias="b")
    sql = f"""
        SELECT
...
def get_avant_view(cursor, origination=None, months=None, competitor=None):
...
def get_competitor_view(cursor, origination=None, months=None, competitor=None):

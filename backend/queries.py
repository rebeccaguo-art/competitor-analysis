...
def _build_filters(
    ...
    # Always exclude noisy/invalid competitors
    clauses.append(f"{alias}competitor NOT IN ('Not Provided', 'Unknown')")
...
def get_available_competitors(cursor) -> list:
    sql = f"SELECT DISTINCT competitor FROM {TABLE_NAME} WHERE competitor IS NOT NULL AND competitor NOT IN ('Unknown', 'Not Provided') ORDER BY competitor"
    rows = run_query(cursor, sql)
...
def get_overall_market_share(cursor, origination=None, months=None, preapproved=True, top_n=None, include_avant=False):
    n = None if top_n == 0 else (top_n or TOP_COMPETITORS_OVERALL)
    ...
    if n is None:
        sql = f"""
            ...
            SELECT month, competitor, share_pct FROM ranked
            ORDER BY month, share_pct DESC
        """
    else:
        avant_case = f"WHEN competitor = '{AVANT_NAME}' THEN competitor" if include_avant else ""
        sql = f"""
            WITH counts AS (
                SELECT month, competitor, COUNT(*) AS cnt
                FROM {TABLE_NAME}
            ...
            grouped AS (
                SELECT month,
                       CASE WHEN rn <= {n} THEN competitor
                            {avant_case}
                            ELSE 'Other' END AS competitor,
                       share_pct
                FROM ranked
            )
            SELECT month, competitor, ROUND(SUM(share_pct), 2) AS share_pct
            FROM grouped
            GROUP BY month, competitor
            ORDER BY month
        """
...
def get_preapproved_market_share(cursor, origination=None, months=None, preapproved=True, top_n=None, include_avant=False):
    n = None if top_n == 0 else (top_n or TOP_COMPETITORS_PREAPPROVED)
...
def get_avg_rank(cursor, origination=None, months=None, preapproved=True, top_n=None, include_avant=False):
    n = None if top_n == 0 else (top_n or TOP_COMPETITORS_RANK)
    where = _build_filters(origination, months, None, preapproved=True)
    ...
    if n is None:
        sql = f"""
            WITH base AS (
                SELECT month,
                       CASE WHEN competitor IN ('Indigo', 'Milestone', 'Destiny') THEN 'Concora'
            ...
            SELECT month, competitor, avg_rank FROM ranked
        ...
        sql = f"""
            ...
            SELECT month, competitor,
                   ROUND(SUM(avg_rank * appearances) / SUM(appearances), 2) AS avg_rank
            FROM grouped
            GROUP BY month, competitor
            ORDER BY month
        """
...
def get_avg_shelf_space(cursor, origination=None, months=None, preapproved=True, top_n=None, include_avant=False):
    n = None if top_n == 0 else (top_n or TOP_COMPETITORS_SHELF)
    where = _build_filters(origination, months, None, preapproved=True)
    ...
    if n is None:
        sql = f"""
            ...
            SELECT month, competitor, avg_shelf_space FROM agg
        ...
        sql = f"""
            ...
            SELECT month, competitor,
                   ROUND(CAST(SUM(total_slots) AS DOUBLE) / SUM(page_count), 2) AS avg_shelf_space
            FROM grouped
            GROUP BY month, competitor
            ORDER BY month
        """
...
def get_preapproved_share(cursor, origination=None, months=None, top_n=None, include_avant=False):
    n = None if top_n == 0 else (top_n or TOP_COMPETITORS_PREAPPROVED)
    ...
    if n is None:
        sql = f"""
            ...
            ORDER BY a.month, share_pct DESC
        """
    else:
        avant_case = f"WHEN competitor = '{AVANT_NAME}' THEN competitor" if include_avant else ""
        sql = f"""
            ...
            combined AS (
                SELECT a.month, a.competitor,
                       a.total,
                       COALESCE(p.pa_total, 0) AS pa_total,
            ...
            SELECT month, competitor,
                   ROUND(SUM(pa_total) * 100.0 / SUM(total), 2) AS share_pct
            FROM grouped
            GROUP BY month, competitor
            ORDER BY month
        """
...
def get_avant_view(cursor, origination=None, months=None, competitor=None, preapproved=True, max_rank=None):
    ...
    sql = f"""
        SELECT
            ...
            MIN(a.annual_fee) AS avant_annual_fee,
        ...
        GROUP BY score_band, avant_af, b.competitor, b.card_name, COALESCE(b.annual_fee, 'N/A')

...
def health():
    ...
    client_id = os.environ.get("DATABRICKS_CLIENT_ID", "NOT SET")
...
def market_overall_share(
    ...
    preapproved: Optional[str] = None,
    top_n: Optional[int] = None,
    include_avant: Optional[str] = None,
):
    pa = preapproved != 'false'
    ia = include_avant == 'true'
    with get_cursor() as cur:
        return queries.get_overall_market_share(cur, origination, parse_months(months), pa, top_n, ia)
...
def market_preapproved_share(
    ...
    ia = include_avant == 'true'
    with get_cursor() as cur:
        return queries.get_preapproved_market_share(cur, origination, parse_months(months), pa, top_n, ia)
...
def market_avg_rank(
    ...
    ia = include_avant == 'true'
    with get_cursor() as cur:
        return queries.get_avg_rank(cur, origination, parse_months(months), pa, top_n, ia)
...
def market_avg_shelf_space(
    ...
    ia = include_avant == 'true'
    with get_cursor() as cur:
        return queries.get_avg_shelf_space(cur, origination, parse_months(months), pa, top_n, ia)
...
def market_card_portfolio(
    ...
    pa = preapproved != 'false'
    with get_cursor() as cur:
        return queries.get_card_portfolio(cur, origination, parse_months(months), pa)
...
def market_preapproved_offers_share(
    ...
    with get_cursor() as cur:
        return queries.get_preapproved_share(cur, origination, parse_months(months), top_n)
...
def shelf_space(
    ...
    pa = preapproved != 'false'
    with get_cursor() as cur:
        return queries.get_shelf_space(cur, origination, parse_months(months), competitor, pa, max_rank)
...
def cooccurrence(
    ...
    pa = preapproved != 'false'
    with get_cursor() as cur:
        return queries.get_cooccurrence_matrix(cur, origination, parse_months(months), pa, max_rank)
...
def cooccurrence_detail(
    ...
    pa = preapproved != 'false'
    with get_cursor() as cur:
        return queries.get_cooccurrence_detail(cur, comp_a, comp_b, origination, parse_months(months), pa, max_rank)
...
def avant_view(
    ...
    pa = preapproved != 'false'
    with get_cursor() as cur:
        return queries.get_avant_view(cur, origination, parse_months(months), competitor, pa, max_rank)
...
def competitor_view(
    ...
    pa = preapproved != 'false'
    with get_cursor() as cur:
        return queries.get_competitor_view(cur, origination, parse_months(months), competitor, pa, max_rank)
